...
// //       className="rounded-start border-0 px-3 py-2"
// //     />
...
// //         className={`mx-1 border-0 px-3 py-2 ${number + 1 === pagination.currentPage ? 'fw-bold shadow-sm' : 'text-primary'}`}
// //         style={{
// //           minWidth: '40px',
// //           display: 'flex',
...
//   Legend,
// } from 'chart.js';
// import { Spinner, Form, Card, Dropdown, Button } from 'react-bootstrap';
// import { FaStore } from 'react-icons/fa';
// import Select from 'react-select';
// import makeAnimated from 'react-select/animated';
// import { IoSearchSharp } from 'react-icons/io5';
...
//     return matchStore && matchDate && matchStructureClient && matchSearch;
//   });
...
//   const charts = generateChartData(groupDataByStore());
//     // Pagination logic
//     const pageCount = Math.ceil(charts.length / itemsPerPage);
//     const offset = currentPage * itemsPerPage;
//     const paginatedUsers = charts.slice(offset, offset + itemsPerPage);
...
//             />
//             <IoSearchSharp className="position-absolute" style={{ top: '10px', left: '10px' }} />
//           </div>
//         </div>
//       </div>
...
const Users = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [usersRes, storesRes, clientsRes] = await Promise.all([
          axios.get(`${process.env.REACT_APP_API_URL}/admin/users`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${process.env.REACT_APP_API_URL}/admin/stores`, {
        ...
        setStoreData(storesRes.data.stores || []);
  ...
  useEffect(() => {
    setCurrentPage(0);
  }, [searchTerm, selectedStores, selectedStructureClients, startDate, endDate]);
  ...
  const filteredUsers = users.filter((user) => {
    const created = new Date(user.created_at);
    const matchStore =
      selectedStores.length === 0 || selectedStores.some((store) => store.value === user.store_name);
    ...
    return matchStore && matchStructureClient && matchDate && matchSearch;
  });
