...
const TokenHistory = () => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchSearch =
      ...
      (!user.store_name && 'Unknown Store'.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const groupDataByStore = () => {
    ...
    filteredUsers.forEach((entry) => {
      const store = entry.store_name || 'Unknown Store'; // Fallback for null store_name
  ...
  const storeOptions = [
    ...
    { value: 'Unknown Store', label: 'Unknown Store' }, // Add option for records with no store
  ];
            ...
            <div className="col-md-4">
              <Form.Group>
                ...
                <Select
                  ...
                  onChange={setSelectedStructureClients} // Updated to handle multiple selections directly
                  components={animatedComponents}
                  placeholder="Select structure clients..."
                  closeMenuOnSelect={false} // Keep menu open after selecting a client
                />
              </Form.Group>
            </div>
          </div>
