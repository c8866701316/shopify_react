...
const Page1 = ({ role }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
    const response =  await axios.post(`${process.env.REACT_APP_API_URL}/store`, newStore, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
...
//     acc_token: "",
//     status: "active",
//     canTrain: true, // Default to true for new stores
//   };
//   const [newStore, setNewStore] = useState(initialStoreState);
...
//       fetchStoreDetails(store.id);
//     } else {
//       toast.error("Training is disabled for this store.");
//     }
//   };
...
//         },
//       });
//       // Set canTrain to true by default for all stores
//       const storesWithCanTrain = response.data.stores.map(store => ({
//         ...store,
//         canTrain: true,
//       }));
//       setStores(storesWithCanTrain);
//     } catch (error) {
//       console.error("Error fetching stores:", error.response ? error.response.data : error.message);
//     } finally {
//       setLoading(false);
...
//   const handlelatestModelClick = async (training_id) => {
//     setLatestModel(true);
//     await fetchLatestTrainingData(training_id);
//     if (intervalId) {
//       clearInterval(intervalId);
//     }
//     const id = setInterval(() => {
...
//   const handleCanTrainChange = (storeId, checked) => {
//     // Update stores state: toggle canTrain for the selected store
//     const updatedStores = stores.map(store => ({
//       ...store,
//       canTrain: store.id === storeId ? !checked : store.canTrain,
//     }));
...
//                           <Button
//                             onClick={() => handleDetailsModalShow(store)}
//                             disabled={!store.canTrain}
//                           >
//                             Training
//                           </Button>
//                         </td>
...
//                           <Form.Check
//                             type="checkbox"
//                             checked={!store.canTrain} // Checked means disabled
//                             onChange={(e) => handleCanTrainChange(store.id, e.target.checked)}
//                             label={store.canTrain ? "Training Enabled" : "Training Disabled"}
//                           />
//                         </td>
//                       </tr>
//                     ))}
...
//             </Form.Group>
//             <Form.Group>
//               <Form.Label>Enable Training</Form.Label>
//               <Form.Check
//                 type="checkbox"
//                 name="canTrain"
//                 checked={newStore.canTrain}
//                 onChange={(e) => setNewStore({ ...newStore, canTrain: e.target.checked })}
...
//       <Modal show={showDetailsModal} onHide={handleDetailsModalClose} size="lg" backdrop="static">
//         <Modal.Header closeButton>
//           <Modal.Title>Training List</Modal.Title>
//         </Modal.Header>
//         <Modal.Body>
...
//                       type="hidden"
//                       id="storeid"
//                       defaultValue={storeDetails.store_id}
//                     />
//                     <Button onClick={addTraining} disabled={!selectedStore?.canTrain}>
//                       Training Now
//                     </Button>
//                   </div>
//                 </div>
